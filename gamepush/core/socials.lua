local M = {}
local core = require("gamepush.core.core")
local helpers = require("gamepush.core.helpers")

---Поддерживается ли шаринг
---@return boolean результат
function M.is_supports_share()
    return core.call_api("gp.socials.isSupportsShare") == true
end

---Проверка поддержки нативного шаринга
---@return boolean результат
function M.is_supports_native_share()
    return core.call_api("gp.socials.isSupportsNativeShare") == true
end

---Поделиться
---@param parameters table|nil
function M.share(parameters)
    helpers.check_table(parameters)
    core.call_api("gp.socials.share", { parameters })
end

---Поддерживается ли нативный постинг
---@return boolean результат
function M.is_supports_native_posts()
    return core.call_api("gp.socials.isSupportsNativePosts") == true
end

---Опубликовать пост
---@param parameters table|nil
function M.post(parameters)
    helpers.check_table(parameters)
    core.call_api("gp.socials.post", { parameters })
end

---Проверка поддержки нативных инвайтов
---@return boolean результат
function M.is_supports_native_invite()
    return core.call_api("gp.socials.isSupportsNativeInvite") == true
end

---Пригласить друзей
---@param parameters table|nil
function M.invite(parameters)
    helpers.check_table(parameters)
    core.call_api("gp.socials.invite", { parameters })
end

---Можно ли приглашать в сообщество на текущей платформе
---@return boolean результат
function M.can_join_community()
    return core.call_api("gp.socials.canJoinCommunity") == true
end

---Поддерживается ли нативное вступление в сообщество
---@return boolean результат
function M.is_supports_native_community_join()
    return core.call_api("gp.socials.isSupportsNativeCommunityJoin") == true
end

---Вступить в сообщество
function M.join_community()
    core.call_api("gp.socials.joinCommunity")
end

---Создать ссылку для шаринга
---@param parameters table|nil
---@return string результат
function M.make_share_url(parameters)
    helpers.check_table_required(parameters)
    return core.call_api("gp.socials.makeShareUrl", { parameters })
end

---Считать параметры при заходе по ссылке
---@param parameters table|nil
---@return string результат
function M.get_share_param(parameter)
    helpers.check_string(parameter, "parameter")
    return core.call_api("gp.socials.getShareParam", { parameter })
end

return M